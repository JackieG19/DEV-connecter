...
{
	private readonly int[] RESTRICTED_VALUES = new int[] {0, 1, 2, 11, 12, 13}; // change int to list
	...
	publiv void ValidateCard(int )
	
	publiv void ValidateBoard()
	{
		// check 1st and las 3 cards for restricted values, swap with a vaild card from the center
		for (int position = 0; position < 3; position++)
		{
			if(RESTRICTED_VALUES.Contains(this.deck.cards[position].val))
			{
				...
				while(true)
				{
					...
					this.deck.cards[newPostion] = temp;
